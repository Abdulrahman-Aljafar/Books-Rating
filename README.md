# MERN App Template repository

## How to use this repo

- Fork this repo and make to become the owner
- In the main directory `/` run `npm install`
- `cd client` and from `client` directory run `npm install`
- Check that there is a .env file in the client directory so that `/client.env` exists and that the content is the following
```shell
